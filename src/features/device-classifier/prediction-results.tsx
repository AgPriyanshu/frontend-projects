...
export default function PredictionResults({
  ...
  const colorClass =
    deviceColors[topPrediction.device as keyof typeof deviceColors];
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div className="space-y-3">
          {predictions.map((prediction, index) => {
            const PredIcon =
              deviceIcons[prediction.device as keyof typeof deviceIcons];
            const predColorClass =
              deviceColors[prediction.device as keyof typeof deviceColors];
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`p-2 rounded-lg ${predColorClass.split(" ")[1]}`}
                  >
                    <PredIcon
                      className={`h-5 w-5 ${predColorClass.split(" ")[0]}`}
                    />
                  </div>
                ...
                <div className="flex items-center space-x-3">
                  <div className="w-24 bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${predColorClass
                        .split(" ")[0]
                        .replace("text-", "bg-")}`}
